...
class ControllerWebBluetooth {
    ...
    connect() {
        return navigator.bluetooth.requestDevice({
            ...
            .then(device => {
                ...
                $("#console").html('Device discovered: <span>' + device.name + '</span>');

                return device.gatt.connect();
            })
            .then(server => {
                console.log('server device: ' + Object.keys(server.device));
                $("#console").html('server device: <span>' + Object.keys(server.device) + '</span>');
            ...
            .catch(error => {
                ...
                $("#console").html('error: <span>' + error + '</span>');
            })
    }
   ...
   /*     let commandWriteChar = requestedCharacteristics.filter((char) => {
        ...
        return server.getPrimaryService(controlService[0].uuid)
            ...
            .then(_ => { */
                ...
                console.log('getting service: ', controlService[0].name);
                $("#console").html('getting service: <span>' + controlService[0].name + '</span>');
 ...
 /*   sendControlService(requestedServices, requestedCharacteristics, server) {
    ...
    handleDeviceDataChanged(event) {
        ...
        var sumTime = 0;
        var timeCount = 0;
        for(var t=0; t < (lastTimes.length - 1); t++){
            lastTimes[t] = lastTimes[t+1];
        }
        lastTimes[lastTimes.length] = thisTime;

        if(lastTimes[0] !== 0){ //if we have enough samples
            for(var s=0; s < (lastTimes.length - 1); s++){
                sumTime = sumTime + ( lastTimes[s+1] - lastTimes[s] );
                timeCount++;
            }
            speed = 1000 / (sumTime / timeCount );
        } 
    ...
    getdeviceData(service, characteristic, server) {
        return server.getPrimaryService(service.uuid)
            .then(newService => {
                ...
                $("#console").html('getting characteristic: <span>' + characteristic.name + '</span>');
